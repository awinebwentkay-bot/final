...
from prompts import PROMOTE_POSTER, PROMOTE_TWEET


def promote_agent(state: ActivityState) -> ActivityState:
    plan = state["activity_plan"]
    print(f"[宣传] 正在生成海报文案...", flush=True)
    poster = llm.invoke(PROMOTE_POSTER.format(plan=plan)).content
    print(f"[宣传] 正在生成推文...", flush=True)
    tweet = llm.invoke(PROMOTE_TWEET.format(plan=plan)).content
